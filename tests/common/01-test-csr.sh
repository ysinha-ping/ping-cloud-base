...
test_no_secret_in_uber_yaml() {
    ...
    # Exclude opensearch-securityconfig and customer-p1-app here since no actual secret data there
    yq 'select(.kind == "Secret" and .metadata.name != "customer-p1-app")' ${uber_yaml_output} -e
